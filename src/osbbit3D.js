...
function initOsbbit3D() {

    const osbbit3D = document.getElementById('osbbit3D');
    const ahya3DWidth = osbbit3D.clientWidth;
    const ahya3DHeight = osbbit3D.clientHeight;
    ...
    loader.load(
        'https://prod.spline.design/bUcPIq38ocmvfnzD/scene.splinecode',
        (splineScene) => {
    ...
    osbbit3D.appendChild(renderer.domElement);
    // scene settings
    ...
    function onWindowResize() {
        let ahya3DWidth = osbbit3D.clientWidth;
        let ahya3DHeight = osbbit3D.clientHeight;
    ...
    osbbit3D.children[0].addEventListener('mouseenter', () => {
        gsap.to(camera.position, { z: zoominZ, duration: duration });
    })
    osbbit3D.children[0].addEventListener('mouseleave', () => {
...
export { initOsbbit3D, OsbbitnextScene }